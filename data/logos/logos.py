...
driver.switch_to.frame(
    driver.find_element_by_xpath("//iframe[contains(@src, 'consent.google.com')]")
)
page_wait_cookies = 5
    ...
    for attempt in range(1, max_attempt + 1):
        try:
            ...
            try:
                ...
                about_this_page = driver.find_element_by_xpath(
                    "/html/body/div/div/b"
                ).get_attribute("innerHTML")
                if about_this_page == "About this page":
                    logger.info(
                        f"reCAPTCHA triggered. Saving checkpoint. Restarting driver."
                    )
                    ...
                    driver.switch_to.frame(
                        driver.find_element_by_xpath(
                            "//iframe[contains(@src, 'consent.google.com')]"
                        )
                    )
                    page_wait_cookies = 5
                    logger.info(
                        f"Waiting { page_wait_cookies } second for page load to accept cookies ..."
                    )
                    time.sleep(page_wait_cookies)
                    driver.find_element_by_xpath(
            ...
            logo_src = driver.find_element_by_xpath(
                "/html/body/div[7]/div[2]/div[9]/div[3]/div/div/div[1]/div[1]/div[2]/div/div[2]/div/div/a/g-img/img"
            ).get_attribute("src")
